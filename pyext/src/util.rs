    ...
    for (style, text) in tokens {
        if remaining > 0 {
            ...
            if remaining >= char_len {
                ...
                // Use char_indices to get the byte index at character position 'remaining'
                let byte_idx = text.char_indices().nth(remaining).map_or(text.len(), |(i, _)| i);
...
pub fn modify_range(
    ...
    for (style, text) in tokens {
        ...
        if range_start_in_text > 0 {
            // Get character indices for slicing
    ...
    pub fn __next__(&mut self) -> Option<(String, String)> {
        ...
        while line_end < self.chars.len() {
            ...
            if ch == '\n' {
                ...
                let ending = String::from_iter(self.chars[line_end..self.index].iter());
